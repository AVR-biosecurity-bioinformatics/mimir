...
# $4 = fasta_files
# $5 = threshold csv
# $6 = clustering threshold to use (either a string or a float)
...
# determine the min sequence identity for the clusters
if [[ $6 == "species_median" ]]; then
	# use median species identity 
	MIN_SEQ_ID=$( awk -F, '(NR>1) && ($1=="species")' $5 | cut -f2 -d, )
elif [[ $6 == "genus_median" ]]; then
	# use median genus identity 
	MIN_SEQ_ID=$( awk -F, '(NR>1) && ($1=="genus")' $5 | cut -f2 -d, )
elif [[ $6 =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	# use value as given 
	if [[ $6 > 1.0 || $6 < 0.0 ]]; then
		echo "Clustering threshold value ${6} is not between 0 and 1 (inclusive)"
		exit 1
	else 
		MIN_SEQ_ID=$6
	fi 
else 
	echo "${6} is an incorrect clustering threshold value: must be 'species_median', 'genus_median' or a float between 0-1 (inclusive)"
	exit 1
fi
